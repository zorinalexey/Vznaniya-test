...
localize() {
	...
	ERROR_uninstall="Ошибка: не удалось удалить пакеты КриптоПро CSP"
	SUCCESS_uninstall="Пакеты КриптоПро CSP успешно удалены"
	MSG_progress="Удаление пакетов КриптоПро CSP..."
    else	
	ERROR_root="Error: this script must be run as root or via sudo"
	ERROR_uninstall="Error: failed to uninstall CryptoPro CSP packages"
	SUCCESS_uninstall="CryptoPro CSP packages have been successfully uninstalled"
	MSG_progress="Uninstalling CryptoPro CSP packages..."
    fi
}
